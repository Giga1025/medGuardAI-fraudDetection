...
def get_total_records():
    response = requests.get(f"{API_URL}?size=1")
...
def fetch_batch(offset, size):
    for attempt in range(MAX_RETRIES):
        try:
            response = requests.get(API_URL, params={'offset': offset, 'size': size}, timeout=120)
            response.raise_for_status()
...
def upload_to_gcs(df, batch_num):
    ...
    blob = bucket.blob(f"raw/cms_partb_2023/batch_{batch_num:06d}.parquet")
